...
function getChartPoints(history) {
  ...
  return history.map((item, index) => {
    const x =
      chartSize.left +
      (chartWidth * index) / Math.max(history.length - 1, 1);
    const y =
      chartSize.top +
      chartHeight -
  ...
  return (
    <div className={styles.evaluation}>
      ...
      <section className={styles.section} aria-labelledby="metrics-history">
        ...
        <article className={`${styles.chartPanel} ${styles[selectedMetric.tone]}`}>
      ...
      <div className={styles.metricCards}>
        {metrics.map((metric) => (
          <article key={metric.id} className={`${styles.metricCard} ${styles[metric.tone]}`}>
